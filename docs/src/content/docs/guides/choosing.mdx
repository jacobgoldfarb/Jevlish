...
- `.by(criterion)` states the selection criterion. It compiles to a Jev Choice question with one option per candidate.
...
`chooseFrom(candidates).by(criterion)` without a `given` in front is a selection value. Put it in `given(x).ask({ ... })` to ask it alongside other questions in the same request. See [Asking several questions](/Jevlish/guides/ask/).

## Evidence

The `choice` record in `judgment.evidence.judgments` has the chosen option id, the confidence, and the full probability distribution over options, so a close call is visible after the fact. See [Evidence and tracing](/Jevlish/guides/evidence/).
