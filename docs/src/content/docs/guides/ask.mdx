...
When several independent things need judging about one subject, `given(x).ask({ ... })` asks them together. One request goes out; each key comes back as its own `Judgment`.
...
- A **selection**: `chooseFrom(candidates).by(criterion)`, optionally `.orNone(...)`. Produces `Judgment<C>` or `Judgment<C | null>`.
